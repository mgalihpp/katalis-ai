...
import { useSettingsStore, periodLabels, TargetPeriod } from '@/store/useSettingsStore';
...
export function TargetEditDrawer({ isOpen, onClose }: TargetEditDrawerProps) {
    ...
    const handleSave = () => {
        ...
        toast.success(`Target ${periodLabels[period].toLowerCase()} berhasil diubah`);
        onClose();
    };
                ...
                <div className="mx-auto w-full max-w-lg flex flex-col overflow-hidden" style={{ maxHeight: '90dvh' }}>
                    <DrawerHeader className="shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DrawerTitle className="text-lg text-left">Ubah Target</DrawerTitle>
                                <p className="text-sm text-muted-foreground">Set target penjualan</p>
                            </div>
                        </div>
                    </DrawerHeader>
                    ...
                    <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4 space-y-5">
                            ...
                            <div className="flex items-center gap-2 overflow-x-auto scrollbar-none pb-1">
                                {periods.map((p) => (
                                    <button
                                        ...
                                        className={`py-2.5 px-2 rounded-xl text-xs font-medium transition-colors ripple ${period === p
                        ...
                        <div className="p-4 bg-primary text-primary-foreground rounded-2xl text-center">
                            <p className="text-sm mb-1">Target {periodLabels[period]} Saat Ini</p>
                            <p className="text-2xl font-bold">{formatRupiah(targetAmount)}</p>
                        </div>

                        {/* Input */}
                        <div>
                            <Label className="text-xs text-muted-foreground">Target Baru</Label>
                            <CurrencyInput
                                value={amount}
                                onChange={setAmount}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {currentPresets.map((preset) => (
                                    <button
                                        ...
                                        onMouseDown={createRippleEffect}
                                        className={`py-2.5 px-3 rounded-xl text-sm font-medium transition-colors ripple ${amount === preset
                                            ? 'bg-primary text-primary-foreground'
                                            : 'bg-muted text-foreground hover:bg-muted/80'
                                            }`}
                                    >
