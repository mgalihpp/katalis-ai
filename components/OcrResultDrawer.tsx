...
import { Check, X, Store, Calendar, Receipt, Trash2, Package, Sparkles } from 'lucide-react';
...
interface OcrResultDrawerProps {
    ...
    onConfirm: (result: OCRReceiptResult) => void;
}
...
export function OcrResultDrawer({
    ...
    useEffect(() => {
        if (result) {
            const items = (result.items || []).map((item) => {
                ...
                const totalPrice = qty > 0 && unitPrice > 0
                    ? qty * unitPrice
            ...
            setReceiptDate(result.receipt_info?.date || new Date().toLocaleDateString('id-ID'));
        }
    }, [result]);
    ...
    const handleUpdateItem = useCallback((id: string, field: keyof OCRItem, value: string | number) => {
        setEditedItems((prev) =>
            prev.map((item) => {
                ...
                if (field === 'quantity' || field === 'unit_price') {
                    const qty = parseOCRNumber(field === 'quantity' ? value : item.quantity);
                    const unitPrice = parseOCRNumber(field === 'unit_price' ? value : item.unit_price);
    ...
    const handleConfirm = () => {
        ...
        const updatedResult: OCRReceiptResult = {
            ...
            items: editedItems.map(({ id, ...item }) => item),
    ...
    return (
        <Drawer open={isOpen} onOpenChange={onClose}>
            <DrawerContent className="max-h-[85dvh]">
                <div className="mx-auto w-full max-w-lg flex flex-col overflow-hidden" style={{ maxHeight: '85dvh' }}>
                    {/* Header */}
                    <DrawerHeader className="shrink-0">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <DrawerTitle className="text-lg text-left">Hasil Scan</DrawerTitle>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <span className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium">Toko</span>
                                        <input
                                            type="text"
                                            value={merchantName}
                                ...
                                <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <span className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium">Tanggal</span>
                                        <input
                                            type="text"
                                            value={receiptDate}
                                        ...
                                        {editedItems.map((item, index) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-start gap-2 mb-2">
                                                    <span className="shrink-0 w-5 h-5 rounded-md bg-primary/10 text-primary text-xs font-medium flex items-center justify-center">
                                                        {index + 1}
                                                    </span>
                                                    <input
                                                        type="text"
